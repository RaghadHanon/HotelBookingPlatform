...
public class GuestServiceTests
{
    ...
    {
        // Arrange
        var requestParameters = new GetBookingsQueryParametersDto { PageNumber = pageNumber, PageSize = pageSize };
        var guest = _fixture.Build<Guest>()
             .Without(g => g.Bookings)
             .Create();
        var expectedBookings = _fixture.Build<Booking>()
        ...
        var expectedPaginatedResult = new PaginatedResult<Booking>(expectedBookings.Skip(skippedBookings).Take(pageSize).ToList(), expectedPaginationMetadata);
        ...
        // Act
        var paginatedResult = await sut.GetAllBookingsForGuestAsync(guest.Id, requestParameters);
    ...
    public async Task GetRecentlyVisitedHotelsAsync_ShouldReturnHotels_WhenValidGuestIdAndCount()
    {
        ...
        var guest = _fixture.Build<Guest>()
             .Without(g => g.Bookings)
             .Create();
        var recentBookings = _fixture.Build<Booking>()
             .Without(b => b.BookingRooms)
             .Without(b => b.Hotel)
             .With(b => b.Guest, guest)
             .CreateMany(count);
        ...
        // Act
        var result = await sut.GetRecentlyVisitedHotelsAsync(guest.Id, count);
