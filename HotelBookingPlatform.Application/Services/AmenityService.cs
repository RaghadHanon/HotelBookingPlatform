...
public class AmenityService : IAmenityService
{
    ...
    public async Task<AmenityOutputDto> CreateAmenity(CreateAndUpdateAmenityDto request)
    {
        ...
        var entity = await _amenityRepository.AddAmenityAsync(amenity);
    ...
    {
        var amenity = await _amenityRepository.GetAmenityAsync(id)
    ...
    {
        var deleted = await _amenityRepository.DeleteAmenityAsync(id);
        if (deleted)
        {
            await _amenityRepository.SaveChangesAsync();
        }
    ...
    {
        var paginatedResult = await _amenityRepository.GetAllAmenitiesAsync(request);
